...
// Try to load from backend root or local
...
const runSimulation = async () => {
    ...
    try {
        ...
        // Use find().sort().limit() and explain
        const topRoomsQuery = Room.find({ isLive: true })
            .sort({ viewerCount: -1 })
            .limit(10);

        const explanation = await (topRoomsQuery as any).explain('executionStats');
        const stats = explanation.executionStats;

        console.log(`Query "Top Live" Execution Time: ${stats.executionTimeMillis}ms`);
        console.log(`Documents Examined: ${stats.totalDocsExamined}`);

        if (stats.totalDocsExamined <= 20) { // Should be exactly 10 if index is perfect, allowing some margin
            console.log('SUCCESS: Compound Index used correctly (minimal docs examined).');
        } else {
            console.warn('WARNING: Query might be scanning. Docs Examined:', stats.totalDocsExamined);
        }
