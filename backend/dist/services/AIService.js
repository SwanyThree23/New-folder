...
class AIService {
     ...
     */
    static async compressPrompt(prompt) {
     ...
     */
    static async analyzeSentiment(chatMessages) {
        if (chatMessages.length === 0)
            return 0.5;
        // Mock sentiment logic: high velocity of messages = higher 'hype' sentiment
        const text = chatMessages.join(' ').toLowerCase();
        let score = 0.5;
        if (text.includes('hype') || text.includes('wow') || text.includes('!'))
            score += 0.3;
        if (text.includes('bad') || text.includes('slow') || text.includes('?'))
            score -= 0.1;
